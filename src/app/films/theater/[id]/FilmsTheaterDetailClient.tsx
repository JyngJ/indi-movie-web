...
function ShowtimeChip({ st, selected, onClick }: { st: Showtime; selected?: boolean; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        padding: '10px 14px', borderRadius: 'var(--radius-xl)', minWidth: 110,
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--spacing-1)' }}>
        <span style={{ fontSize: 'var(--text-title)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: 'var(--color-text-primary)' }}>
          {st.showTime.slice(0, 5)}
        </span>
        {st.endTime && (
          <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-caption)', fontFeatureSettings: '"tnum"' }}>
      ...
      {st.seatTotal > 0 && (
        <div style={{ marginTop: 3, fontSize: 'var(--text-caption)', fontFeatureSettings: '"tnum"' }}>
          ...
          {low && !soldout && <span style={{ marginLeft: 4, fontSize: 'var(--text-caption)', color: 'var(--color-warning)', fontWeight: 600 }}>잔여↓</span>}
          {soldout && <span style={{ marginLeft: 4, fontSize: 'var(--text-caption)', color: 'var(--color-error)', fontWeight: 700 }}>매진</span>}
        </div>
      )}
      {label && (
        <div style={{ marginTop: 2, fontSize: 'var(--text-caption)', color: 'var(--color-text-caption)' }}>{label}</div>
      )}
    </div>
  )
}
  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          width: '100%', display: 'flex', alignItems: 'flex-start', gap: 'var(--spacing-3)',
          padding: '14px 16px', background: 'none', border: 'none', cursor: 'pointer',
          textAlign: 'left', minHeight: 'auto',
        }}
      >
        {/* 포스터 */}
        <div style={{ width: posterW, height: posterH, borderRadius: 'var(--radius-md)', overflow: 'hidden', flexShrink: 0, position: 'relative', backgroundColor: 'var(--color-surface-raised)' }}>
          {movie.posterUrl ? (
        ...
        <div style={{ flex: 1, minWidth: 0, paddingTop: 2 }}>
          <span style={{ fontSize: 'var(--text-title)', fontWeight: 700, color: 'var(--color-text-primary)', fontFamily: 'var(--font-serif)', display: 'block', lineHeight: 1.3, wordBreak: 'keep-all' }}>
            {normalizeTitle(movie.title)}
          </span>
          {meta && (
            <span style={{ marginTop: 5, display: 'block', fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)', lineHeight: 1.5 }}>
              {meta}
            </span>
          )}
          {movie.genre.length > 0 && (
            <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-1)' }}>
              {movie.genre.slice(0, 2).map((g) => (
                <span key={g} style={{ fontSize: 'var(--text-caption)', padding: '2px 8px', borderRadius: 99, border: '1px solid var(--color-border)', color: 'var(--color-text-caption)', backgroundColor: 'var(--color-surface-raised)' }}>
                  {g}
                </span>
              ))}
            </div>
      ...
      {/* 상영시간 */}
      <div style={{ padding: '0 16px 14px', display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-2)' }}>
        {showtimes.map((st) => (
          <ShowtimeChip
            key={st.id} st={st}
...
export function FilmsTheaterDetailClient({ theater }: { theater: Theater }) {
  ...
  const btnBase: React.CSSProperties = {
    ...
    height: 40, padding: '0 16px', borderRadius: 'var(--radius-xl)',
    ...
    <div style={{ paddingBottom: isDesktop ? (selectedShowtimeData ? 220 : 64) : (selectedShowtimeData ? 148 : 80) }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 10px', borderRadius: 99, border: '1px solid color-mix(in srgb, var(--color-primary-base) 55%, transparent)', backgroundColor: 'var(--color-primary-subtle-l)', marginBottom: 10 }}>
          <span style={{ fontSize: 'var(--text-caption)', fontWeight: 600, color: 'var(--color-primary-base)' }}>독립·예술영화관</span>
        </div>
        ...
        <button
          onClick={copyAddress}
          style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-1-5)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: 'var(--color-text-sub)', fontSize: 'var(--text-meta)', marginBottom: 18, minHeight: 'auto' }}
        >
          <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" />
          </svg>
          <span>{theater.address}</span>
          <span style={{ marginLeft: 2, display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', color: copied ? 'var(--color-primary-base)' : 'var(--color-text-caption)', fontSize: 'var(--text-caption)', fontWeight: 500 }}>
            <IcoCopy />
            {copied ? '복사됨' : '복사'}
          </span>
        </button>
          ...
          {dates.map((d, i) => {
            ...
            return (
              <button
                ...
                disabled={!hasShows}
              >
                <span style={{ fontSize: 'var(--text-caption)', fontWeight: 500, color: isSelected ? 'var(--color-primary-base)' : isHoliday ? 'var(--color-error)' : 'var(--color-text-caption)' }}>
                  {i === 0 ? '오늘' : day}
                </span>
                <span style={{ fontSize: 'var(--text-h3)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: isSelected ? 'var(--color-primary-base)' : isHoliday ? 'var(--color-error)' : 'var(--color-text-primary)' }}>
                  {date}
                </span>
              </button>
            )
      ...
      <div style={{ padding: isDesktop ? '20px 28px 0' : '16px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontSize: 'var(--text-h3)', fontWeight: 700, color: 'var(--color-text-primary)' }}>
            🎬 현재 상영중{' '}
            <span style={{ fontSize: 'var(--text-title)', color: 'var(--color-primary-base)' }}>{movieShowtimeGroups.length}편</span>
          </span>
        </div>

        {isLoading ? (
          <div style={{ height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)' }}>
            불러오는 중…
          </div>
        ) : movieShowtimeGroups.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0', fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)' }}>
            이 날 상영 정보가 없습니다
          </div>
        ) : (
    ...
    return (
      <div style={{ minHeight: '100svh', backgroundColor: 'var(--color-surface-bg)' }}>
        ...
        <div style={{ position: 'sticky', top: 0, zIndex: 50, paddingTop: 'env(safe-area-inset-top)', backgroundColor: 'var(--color-surface-bg)', borderBottom: '1px solid var(--color-border)' }}>
          <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 4, paddingRight: 12, gap: 'var(--spacing-1-5)', maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', minWidth: 0 }}>
              ...
              <button onClick={() => router.push('/films')} style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', minHeight: 'auto', flexShrink: 0 }}>영화</button>
              <span style={{ color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', flexShrink: 0 }}>&gt;</span>
              <span style={{ fontSize: 'var(--text-meta)', fontWeight: 600, color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{theater.name}</span>
            </div>
            <RegionFilterWidget />
          </div>
        </div>
        ...
        {selectedShowtimeData && (
          <div style={{ position: 'fixed', right: 32, bottom: 32, width: 300, zIndex: 100, borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', boxShadow: '0 6px 24px color-mix(in srgb, var(--color-primary-base) 55%, transparent)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 14px 16px', display: 'flex', flexDirection: 'column', gap: 'var(--spacing-2-5)' }}>
              ...
              <div>
                <div style={{ fontSize: 'var(--text-body)', fontWeight: 700, color: 'var(--color-text-primary)', fontFamily: 'var(--font-serif)', lineHeight: 1.3 }}>{selectedShowtimeData.movieTitle}</div>
                <div style={{ marginTop: 3, fontSize: 'var(--text-meta)', fontWeight: 700, color: 'var(--color-primary-base)' }}>{theater.name}</div>
              </div>
