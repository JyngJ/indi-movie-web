...
function ShowtimeChip({ st, selected, onClick }: { st: Showtime; selected?: boolean; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        padding: '10px 14px', borderRadius: 'var(--radius-xl)', minWidth: 100,
    ...
    >
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--spacing-1)' }}>
        <span style={{ fontSize: 'var(--text-title)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: 'var(--color-text-primary)' }}>{st.showTime.slice(0, 5)}</span>
        {st.endTime && <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-caption)', fontFeatureSettings: '"tnum"' }}>-{st.endTime.slice(0, 5)}</span>}
      </div>
      {st.seatTotal > 0 && (
        <div style={{ marginTop: 3, fontSize: 'var(--text-caption)', fontFeatureSettings: '"tnum"' }}>
          ...
          {soldout && <span style={{ marginLeft: 4, fontSize: 'var(--text-caption)', color: 'var(--color-error)', fontWeight: 700 }}>매진</span>}
        </div>
      )}
    </div>
  )
...
function DirectorChip({ name, photoUrl, onClick }: { name: string; photoUrl?: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--spacing-2)', padding: '6px 12px 6px 6px', borderRadius: 'var(--radius-full)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', cursor: 'pointer', minHeight: 'auto' }}>
      ...
      <span style={{ fontSize: 'var(--text-meta)', fontWeight: 600, color: 'var(--color-text-primary)' }}>{name}</span>
      <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-primary-base)', fontWeight: 500 }}>감독 →</span>
    </button>
  )
}
...
export function FilmsMovieDetailClient({ movie }: { movie: MovieDetail }) {
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
        ...
        {movie.genre.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-1-5)', marginTop: 12 }}>
            {movie.genre.map((g) => (
              <span key={g} style={{ height: 24, padding: '0 10px', display: 'inline-flex', alignItems: 'center', borderRadius: 'var(--radius-full)', fontSize: 'var(--text-meta)', fontWeight: 500, backgroundColor: 'var(--color-primary-subtle-l)', border: '1px solid color-mix(in srgb, var(--color-primary-base) 40%, transparent)', color: 'var(--color-primary-base)' }}>
                {g}
              </span>
            ))}
          </div>
        ...
        {meta && (
          <div style={{ marginTop: 10, fontSize: 'var(--text-meta)', color: 'var(--color-text-sub)' }}>{meta}</div>
        )}
        {movie.director.length > 0 && (
          <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-2)' }}>
  ...
  const ctaButtons = (
    <div style={{ padding: isDesktop ? '0 0 20px' : '12px 16px', display: 'flex', gap: 'var(--spacing-2)' }}>
      <button
        onClick={() => router.push(`/?movie=${movie.id}`)}
        style={{ flex: 1, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-primary-base)', backgroundColor: 'var(--color-primary-base)', color: '#fff', fontSize: 'var(--text-body)', fontWeight: 600, cursor: 'pointer' }}
      >
        <IcoMap /> 지도에서 상영관 필터로 보기
      </button>
      <button
        onClick={() => navigator.share?.({ title: movie.title, url: window.location.href }).catch(() => {})}
        style={{ width: 42, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', color: 'var(--color-text-body)', cursor: 'pointer', flexShrink: 0 }}
      >
        <IcoShare />
      </button>
    </div>
  ...
  const synopsisSection = movie.synopsis ? (
    <div style={{ padding: isDesktop ? '0 0 20px' : '0 16px 16px', borderBottom: '1px solid var(--color-border)' }}>
      <p style={{ margin: '0 0 8px', fontSize: 'var(--text-meta)', fontWeight: 500, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-text-caption)' }}>시놉시스</p>
      <p style={{ margin: 0, fontSize: 'var(--text-body)', lineHeight: 1.8, color: 'var(--color-text-body)', wordBreak: 'keep-all' }}>
        {movie.synopsis}
      </p>
    </div>
  ) : null
    ...
    return (
      <div key={entry.theaterId} style={{ borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', overflow: 'hidden' }}>
        <button
          ...
          style={{ width: '100%', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 'var(--spacing-2-5)', padding: '14px 16px 12px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', minHeight: 'auto', borderBottom: '1px solid var(--color-border)' }}
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 4, display: 'flex', alignItems: 'flex-start', gap: 'var(--spacing-1)', color: 'var(--color-text-sub)', fontSize: 'var(--text-meta)' }}>
              <IcoPin /><span style={{ wordBreak: 'keep-all', lineHeight: 1.45 }}>{entry.theaterAddress}</span>
            </div>
          </div>
          <IcoChevronRight />
        </button>
        <div style={{ padding: '10px 16px 14px', display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-2)' }}>
  ...
  const showtimesSection = (
    <div style={{ paddingTop: 8 }}>
      <div style={{ padding: isDesktop ? '16px 0 0' : '16px 16px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button
          onClick={() => router.push(`/?movie=${movie.id}`)}
          style={{ height: 30, padding: '0 12px', borderRadius: 99, border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', color: 'var(--color-text-body)', fontSize: 'var(--text-meta)', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', minHeight: 'auto' }}
        >
          <IcoMap /> 지도에서 필터로 보기
        </button>
      </div>
      {!isLoading && totalTheaterCount > 0 && (
        <p style={{ margin: 0, padding: isDesktop ? '6px 0 0' : '6px 16px 0', fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)', lineHeight: 1.5 }}>
          {regionId && (
          ...
          {dates.map((d, i) => {
            ...
            return (
              <button
                ...
                disabled={!hasShows}
              >
                <span style={{ fontSize: 'var(--text-caption)', fontWeight: 500, color: isSelected ? 'var(--color-primary-base)' : isHoliday ? 'var(--color-error)' : 'var(--color-text-caption)' }}>{i === 0 ? '오늘' : day}</span>
                <span style={{ fontSize: 'var(--text-h3)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: isSelected ? 'var(--color-primary-base)' : isHoliday ? 'var(--color-error)' : 'var(--color-text-primary)' }}>{date}</span>
              </button>
            )
          })}
        </div>
      ...
      {/* 극장별 목록 */}
      <div style={{ padding: isDesktop ? '16px 0 64px' : `12px 16px ${selectedShowtimeData ? 148 : 52}px`, display: 'flex', flexDirection: 'column', gap: 'var(--spacing-3)' }}>
        {isLoading ? (
          <div style={{ height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)' }}>불러오는 중…</div>
        ) : dayTheaters.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0', fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)' }}>이 날 상영 정보가 없습니다</div>
        ) : regionId ? (
          <>
            {inRegionEntries.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '16px 0', fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)' }}>{regionId} 지역 상영 정보가 없습니다</div>
            ) : (
            ...
            {otherRegionEntries.length > 0 && (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-2)', margin: '4px 0' }}>
                  ...
                  <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-caption)', fontWeight: 500, whiteSpace: 'nowrap' }}>{regionId} 외 지역 영화관</span>
                  <div style={{ flex: 1, height: 1, backgroundColor: 'var(--color-border)' }} />
                </div>
  ...
  const detailInfoSection = (
    <div>
      <p style={{ margin: '0 0 8px', fontSize: 'var(--text-meta)', fontWeight: 500, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-text-caption)' }}>상세 정보</p>
  ...
  const directorSideCard = movie.director.length > 0 ? (
    <div>
      <p style={{ margin: '0 0 8px', fontSize: 'var(--text-meta)', fontWeight: 500, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-text-caption)' }}>감독</p>
      {movie.director.map((name) => (
  ...
  const desktopBookingCard = selectedShowtimeData ? (
    <div style={{ borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', boxShadow: '0 6px 24px color-mix(in srgb, var(--color-primary-base) 55%, transparent)', overflow: 'hidden' }}>
      <div style={{ padding: '14px 14px 16px', display: 'flex', flexDirection: 'column', gap: 'var(--spacing-2-5)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 'var(--text-caption)', fontWeight: 700, letterSpacing: '0.5px', color: 'var(--color-text-caption)' }}>회차 선택됨</span>
        ...
        <div>
          <div style={{ fontSize: 'var(--text-body)', fontWeight: 700, color: 'var(--color-text-primary)', fontFamily: 'var(--font-serif)', lineHeight: 1.3 }}>{movie.title}</div>
          <div style={{ marginTop: 3, fontSize: 'var(--text-meta)', fontWeight: 700, color: 'var(--color-primary-base)' }}>{selectedShowtimeData.theaterName}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--spacing-1-5)' }}>
          <span style={{ fontSize: 'var(--text-h1)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: 'var(--color-text-primary)' }}>{selectedShowtimeData.st.showTime.slice(0, 5)}</span>
          {selectedShowtimeData.st.endTime && <span style={{ fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)', fontFeatureSettings: '"tnum"' }}>→ {selectedShowtimeData.st.endTime.slice(0, 5)}</span>}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-1)' }}>
          {([
            ...
            <div key={row.key} style={{ display: 'flex', gap: 'var(--spacing-2)', fontSize: 'var(--text-meta)' }}>
        ...
        {selectedShowtimeData.st.bookingUrl ? (
          <a href={selectedShowtimeData.st.bookingUrl} target="_blank" rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 44, borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--color-primary-base)', color: '#fff', fontSize: 'var(--text-body)', fontWeight: 700, textDecoration: 'none', marginTop: 4 }}>
            예매하러 가기 →
          </a>
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 40, borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--color-surface-raised)', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', border: '1px solid var(--color-border)', marginTop: 4 }}>
            예매 링크 없음
          </div>
        )}
      </div>
    ...
    return (
      <div style={{ minHeight: '100svh', backgroundColor: 'var(--color-surface-bg)' }}>
        ...
        <div style={{ position: 'sticky', top: 0, zIndex: 50, paddingTop: 'env(safe-area-inset-top)', backgroundColor: 'var(--color-surface-bg)', borderBottom: '1px solid var(--color-border)' }}>
          <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 4, paddingRight: 12, gap: 'var(--spacing-1-5)', maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', minWidth: 0 }}>
              ...
              <button onClick={() => router.push('/films')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', minHeight: 'auto' }}>영화</button>
              <span style={{ color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)' }}>&gt;</span>
              <span style={{ fontSize: 'var(--text-meta)', fontWeight: 600, color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{movie.title}</span>
            </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 32, alignItems: 'flex-start' }}>
            ...
            {/* sidebar */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-5)', paddingTop: 8, position: 'sticky', top: 68 }}>
              {desktopBookingCard}
              {directorSideCard}
              {detailInfoSection}
            </div>
    ...
    <div className="page-slide-in" style={{ minHeight: '100svh', backgroundColor: 'var(--color-surface-bg)' }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 50, paddingTop: 'env(safe-area-inset-top)', backgroundColor: 'var(--color-surface-bg)' }}>
        <div style={{ height: 52, display: 'flex', alignItems: 'center', borderBottom: '1px solid var(--color-border)', backgroundColor: 'var(--color-primary-subtle-l)' }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 'var(--spacing-1)', fontSize: 'var(--text-meta)', minWidth: 0 }}>
            <button onClick={() => router.push('/films')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', minHeight: 'auto', padding: 0, flexShrink: 0 }}>영화</button>
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, backgroundColor: 'var(--color-surface-card)', borderTop: '1px solid var(--color-border)', padding: '12px 16px', paddingBottom: 'max(16px, env(safe-area-inset-bottom))', boxShadow: '0 -4px 20px rgba(0,0,0,0.12)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontSize: 'var(--text-subtitle)', fontWeight: 700, fontFeatureSettings: '"tnum"', color: 'var(--color-text-primary)' }}>
                {selectedShowtimeData.st.showTime.slice(0, 5)}
                {selectedShowtimeData.st.endTime && <span style={{ fontSize: 'var(--text-meta)', color: 'var(--color-text-caption)', marginLeft: 6, fontWeight: 400 }}>→ {selectedShowtimeData.st.endTime.slice(0, 5)}</span>}
              </div>
              <div style={{ marginTop: 2, fontSize: 'var(--text-meta)', color: 'var(--color-text-sub)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          ...
          {selectedShowtimeData.st.bookingUrl ? (
            <a href={selectedShowtimeData.st.bookingUrl} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 48, borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--color-primary-base)', color: '#fff', fontSize: 'var(--text-subtitle)', fontWeight: 700, textDecoration: 'none' }}>
              예매하러 가기 →
            </a>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 48, borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--color-surface-raised)', color: 'var(--color-text-caption)', fontSize: 'var(--text-meta)', border: '1px solid var(--color-border)' }}>
              예매 링크 없음
            </div>
          )}
        </div>
...
function DirectorSideCard({ name, onClick }: { name: string; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 'var(--spacing-3)', padding: '12px 14px', borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)', cursor: 'pointer', textAlign: 'left', minHeight: 'auto', marginBottom: 8 }}
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginTop: 2, fontSize: 'var(--text-caption)', color: 'var(--color-primary-base)', fontWeight: 500 }}>감독 페이지 보기 →</div>
      </div>
      <IcoChevronRight />
    </button>
  )
