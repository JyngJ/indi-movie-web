...
export default function FavoritesPage() {
  ...
  const list = tab === 'movie' ? summary.movies : summary.theaters
    ...
    <MyPageShell title="관심 목록" onBack={() => router.push('/my')}>
      ...
      {empty && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '48px 16px', textAlign: 'center' }}>
          ...
          <Link href={tab === 'movie' ? '/' : '/map'} style={{ textDecoration: 'none' }}>
            <Button variant="secondary" size="md">{tab === 'movie' ? '상영작 둘러보기' : '지도에서 극장 찾기'}</Button>
          </Link>
        </div>
      )}
