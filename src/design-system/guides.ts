...
export const GUIDES: Record<string, ComponentGuide> = {
  ...
  PosterChip: {
    ...
    specs: [
      ...
      { title: '코너 태그와 떠 있는 칩', desc: '칩의 값이 (영화, 오늘 날짜) 두 가지만으로 정해지면 코너 태그로 붙입니다. 극장·회차·보는 사람의 위치 중 하나라도 있어야 정해지면 떠 있는 칩으로 둡니다. 코너 태그는 포스터에 인쇄된 꼬리표라 테두리와 한 몸이고 그림자가 없습니다. 떠 있는 칩은 지금 보는 화면이 포스터 위에 얹은 메모라 그림자를 둔 별도 층입니다.' },
      { title: '판별 질문', desc: '① 이 포스터를 다른 섹션이나 다른 극장 화면으로 옮겨도 같은 값인가? ② 서울에 있는 사람과 부산에 있는 사람이 같은 값을 보는가? 둘 다 예면 코너 태그, 하나라도 아니오면 떠 있는 칩입니다. 코너 태그: 개봉 주년(영화 연도·올해) · 막바지 D-N(영화·오늘). 떠 있는 칩: 매진(회차) · 회차 시각(극장·회차) · 거리(극장·사용자 위치) · GV 유형(GV 행사).' },
      { title: '경계 사례', desc: '막바지 D-N은 전국 모든 극장 중 가장 늦은 상영일로 계산하므로 코너 태그입니다. "이 극장에서의 마지막 상영"을 보여 준다면 극장이 있어야 정해지는 값이라 떠 있는 칩이 됩니다. 지역 필터를 켜도 D-N은 전국 기준을 유지합니다 — 코너 태그는 영화의 사실을 말하고, 지역 사정은 극장 시트의 시간표가 보여 줍니다. 순위는 (영화, 오늘)로 정해지지만 칩이 아니라 좌하단 스크림과 큰 숫자라는 전용 형식이라 이 기준 밖입니다. 영화제 상태 칩("진행 중")은 기준상 코너 태그 후보지만 아직 떠 있는 칩으로 남아 있습니다.' },
  ...
  Toast: {
    ...
    usage: [
      ...
      { kind: 'dont', rule: '토스트에 닫기 아이콘을 붙이지 않습니다. 사용자가 손으로 치워야 하는 알림이면 애초에 토스트가 아닙니다.', instead: '스스로 사라지게 둡니다.' },
      { kind: 'caution', rule: '사라진 뒤 되돌릴 길이 화면에 남아 있는지 확인합니다. 지도 지역 안내는 필터 칩이 그 역할을 하므로 토스트로 충분합니다.', instead: '되돌릴 길이 화면에 없으면 시트나 모달처럼 남는 자리를 씁니다.' },
    ],
    a11y: [
