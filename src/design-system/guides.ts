...
export const GUIDES: Record<string, ComponentGuide> = {
  ...
  PosterThumb: {
    intro: '포스터 자리입니다. 비율(2:3)과 라운드, 이미지가 없을 때의 대체 화면을 한곳에서 정합니다. 포스터는 화면마다 크기가 다르지만 비율과 모서리는 같아야 합니다.',
  },

  DropdownRow: {
    intro: '드롭다운 안의 한 줄입니다. 선택 표시는 오른쪽 체크로만 나타내고, 줄 전체가 누를 수 있는 영역입니다.',
  },

  PanelShell: {
    intro: '데스크톱 상세 패널의 껍데기입니다. 뒤로·닫기 버튼과 스크롤 영역을 담아, 안에 들어가는 내용은 자기 일만 하면 됩니다.',
  },

  Carousel: {
    intro: '가로로 미는 목록입니다. embla를 감싸 스냅·화살표·인디케이터를 붙입니다. 조각(Content·Item·Navigation·Indicator)은 이 안에서만 씁니다.',
  },
