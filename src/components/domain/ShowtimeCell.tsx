...
export function ShowtimeCell({
  ...
  return (
    ...
    >
      ...
      >
        {isNowPlaying ? (
          /* 글자 뒤 게이트 빛 + 글자 자체의 빛 흔들림 (globals .projector-gate).
             color는 클래스가 소유한다 — 인라인이면 애니메이션과 출처가 갈려 읽기 어렵다 */
          <span className="projector-gate" style={{ fontSize: 'var(--text-body)', fontWeight: 700 }}>지금 상영중</span>
        ) : isEnded ? (
