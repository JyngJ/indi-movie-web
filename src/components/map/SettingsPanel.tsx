...
function AboutPage() {
  const team = [
    { name: '정재용', role: 'Design · Frontend', linkedin: 'https://www.linkedin.com/in/jaeyongjung/' },
    { name: '정재현', role: 'Database · Backend', linkedin: null },
  ]
  return (
      ...
      <div style={{ margin: '0 16px', borderRadius: 12, overflow: 'hidden', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface-card)' }}>
        ...
        {team.map((member, i) => (
          <div key={member.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: i < team.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: 'var(--color-primary-subtle-l)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--color-primary-base)' }}>
              {member.name[1]}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
