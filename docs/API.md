...
> **[스테일 주의 — 2026-08-14 검토]** 작성 시점(백엔드 결정 전)의 인터페이스 계약 초안. 실제 구현은 Next.js route handler(`src/app/api/`) + Supabase 클라이언트 직접 조회 혼합이며 아래 스펙과 경로·형태가 다르다. 현재 API 파악은 `src/app/api/` 소스를 직접 읽을 것.
