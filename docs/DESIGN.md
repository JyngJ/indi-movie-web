...
- **코너 태그(`attached`)** — 값이 영화 한 편에 하나로 정해지면 포스터 모서리에 붙인다(개봉 주년 · 막바지 D-N). 극장·회차·위치마다 달라지는 값(매진 · 회차 시각 · 거리 · GV 유형)은 떠 있는 칩으로 둔다. offset 0, 바깥 모서리 `--radius-poster`, 안쪽 대각 모서리만 `--radius-badge`, 그림자 없음 (2026-09-27)
