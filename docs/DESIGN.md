...
- **코너 태그(`attached`)** — 칩 값이 (영화, 오늘 날짜)만으로 정해지면 포스터 모서리에 붙인다(개봉 주년 · 막바지 D-N). 극장·회차·보는 사람의 위치 중 하나라도 필요하면 떠 있는 칩으로 둔다(매진 · 회차 시각 · 거리 · GV 유형). 판별 질문: 다른 섹션·극장 화면으로 옮겨도 같은 값인가, 서울과 부산의 사용자가 같은 값을 보는가 — 둘 다 예면 코너. 막바지 D-N은 전국 최종 상영일 기준이라 코너이고, 극장별 마지막 상영을 보여 준다면 떠 있는 칩이다. 순위는 전용 형식(좌하단 스크림+숫자)이라 이 기준 밖. offset 0, 바깥 모서리 `--radius-poster`, 안쪽 대각 모서리만 `--radius-badge`, 그림자 없음 (2026-09-27)
- 코너 태그는 **좌상단 한 자리만** 쓴다. 우상단은 관심 하트 자리. 막바지 D-N과 개봉 주년이 겹치면 D-N 우선(날짜가 지나면 사라지는 값이라)
- 코너 태그 색은 scrim(먹색 반투명)이 기본. 흰 반투명은 밝은 포스터에서, 회색 솔리드는 흑백 포스터에서 묻혔다
- D-day: 오늘=error, D-1=warning, D-2+=scrim (코너 태그). 떠 있는 칩의 그림자는 0 1px 4px rgba(0,0,0,0.35)
