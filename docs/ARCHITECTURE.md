...
> **[일부 스테일 — 2026-08-14 검토]** 설계 원칙·데이터 레이어 절은 유효하나, 아래 폴더 구조는 초기 설계안이다. 실제 구조와 주요 차이: `my/`·`auth/`(로그인)는 만들지 않았고, 진입점은 `(tabs)` 라우트 그룹(`'/'` = 상영작 리스트, `/map` = 지도, `/faq`, `/more`)이며, 극장·감독·지역 상세는 `films/theater/[id]`·`films/director/[name]`·`films/area/[id]`에 있다. 그 외 `admin/`(어드민 콘솔), `festival/[slug]`, `p/`(포트폴리오 유입 추적), `dev/`(컴포넌트 갤러리)가 추가됐다. 정확한 현재 구조는 `src/app/` 트리를 직접 볼 것.
