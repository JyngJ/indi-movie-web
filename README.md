...
전국 각지에 흩어진 독립·예술영화관의 상영 정보를 한 곳에서 찾을 수 없다는 불편함에서 시작한 프로젝트입니다. 디자이너 1인이 기획·디자인을 담당하고, 개발자 1인이 합류해 프론트엔드·백엔드·데이터 파이프라인을 함께 구현했습니다. AI 코딩 도구(Claude Code)를 설계 단계부터 구현까지 실무 흐름에 통합해 소규모 팀의 개발 속도와 코드 품질을 함께 높이는 방식을 실험했습니다.
...
**상영작 탭 (기본 진입 `/`)**
- 전국 독립·예술영화관 상영작을 리스트로 탐색 — 큐레이션 섹션·랭킹·지역(`/films/area/[지역]`)·극장·감독 상세 페이지
- 검색엔진 대응: 지역·극장·감독·영화 상세가 서버 렌더링 + JSON-LD + sitemap·IndexNow 자동 통보

**지도 기반 탐색 (`/map`)**
- 전국 독립·예술영화관 핀 표시, 클러스터링, 극장 선택 시 바텀시트 슬라이드 인
...
- 크롤러는 스케줄드 서버(Raspberry Pi) crontab으로 주기 실행 — 좌석/상영시간표/큐레이션. 운영·장애 대응(예매 API IP 밴 복구 포함)은 [`docs/RUNBOOK-crawler.md`](docs/RUNBOOK-crawler.md) 참조
